package com.epiphyllum.learnmvc.chapter6.web.controller.support.validator;
...
public class UserModelValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		ValidationUtils.rejectIfEmpty(errors, "username", "username.not.empty");    // 帮我们添加个逻辑????
		...
		for(String forbiddenWord : FORBINDDDEN_WORD_SET) {
			if(user.getUsername().contains(forbiddenWord)) {
				//用户名包含屏蔽关键字
				// 第三个以及以后的参数是username.forbidden的模板参数
